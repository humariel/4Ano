...
function recSubdivisionCentroid( v1, v2, v3, 
	...
	if( recursionDepth == 0 ) {
        ...
        var colorCentroid = computeCentroid( c1, c2, c3 );
        
        // 3 recursive calls 

        recSubdivisionCentroid( v1, centroid, v2, c1, colorCentroid, c2,
                                coordsArray, colorsArray, recursionDepth - 1 );

        recSubdivisionCentroid( v2, centroid, v3, c2, colorCentroid, c2,
                                coordsArray, colorsArray, recursionDepth - 1 );

        recSubdivisionCentroid( v3, centroid, v1, c3, colorCentroid, c1,
                                coordsArray, colorsArray, recursionDepth - 1 );
	}
}
...
function moveToSphericalSurface( coordsArray ) {
	...
	for(index = 0; index < coordsArray.length; index += 3){
		v = normalize(coordsArray.slice(index, index + 3))
		for(i = 0; i < 3; i++)
			coordsArray[i + index] = v[i];
	}
	
}


function computeAllVersors(coordsArray){
	var versorsArray = [];
	var pontos;
	var p0, p1, p2;

	for(tindex=0; tindex < coordsArray.length; tindex += 9){
		pontos = coordsArray.slice(tindex, tindex+9);
		p0 = pontos.slice(0,3);
		p1 = pontos.slice(3,6);
		p2 = pontos.slice(6,9);
		versorsArray.push([0, 0, 0]);
		versorsArray.push(computeVersor(p0, p1, p2));
	}
	return versorsArray;
}