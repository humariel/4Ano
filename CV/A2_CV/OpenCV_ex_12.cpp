...
int main( int argc, char** argv )
{
    ...
    cv::cvtColor(imagem, gray_img, cv::COLOR_RGB2GRAY);
    //Zeros matrix
    Mat g, red, green, blue;
    g = Mat::zeros(Size(imagem.cols, imagem.rows), CV_8UC1);
    ...
    //imagem vermelha
    vector<Mat> channelsRed;
    channelsRed.push_back(g);
    channelsRed.push_back(g);
    channelsRed.push_back(rgbChannels[2]);
    cv::merge(channelsRed, red);
    namedWindow("Imagem em Vermelho", cv::WINDOW_AUTOSIZE);
    imshow("Imagem em Vermelho", red);
    Mat histogramaRed = computeHistogram( rgbChannels[2] );
    Mat histImageRed = createHistogramImage( histogramaRed );
    namedWindow( "Histograma Red", cv::WINDOW_AUTOSIZE );
    imshow( "Histograma Red", histImageRed);
    

    //imagem verde
    vector<Mat> channelsGreen;
    channelsGreen.push_back(g);
    channelsGreen.push_back(rgbChannels[1]);
    channelsGreen.push_back(g);
    cv::merge(channelsGreen, green);
    namedWindow("Imagem em Verde", cv::WINDOW_AUTOSIZE);
    imshow("Imagem em Verde", green);
    Mat histogramaGreen = computeHistogram( rgbChannels[1] );
    Mat histImageGreen = createHistogramImage( histogramaGreen );
    namedWindow( "Histograma Green", cv::WINDOW_AUTOSIZE );
    imshow( "Histograma Green", histImageGreen);

    //imagem azul
    vector<Mat> channelsBlue;
    channelsBlue.push_back(rgbChannels[0]);
    channelsBlue.push_back(g);
    channelsBlue.push_back(g);
    cv::merge(channelsBlue, blue);
    namedWindow("Imagem em Azul", cv::WINDOW_AUTOSIZE);
    imshow("Imagem em Azul", blue);
    Mat histogramaBlue = computeHistogram( rgbChannels[0] );
    Mat histImageBlue = createHistogramImage( histogramaBlue);
    namedWindow( "Histograma Blue", cv::WINDOW_AUTOSIZE );
    imshow( "Histograma Blue", histImageBlue);

    //iamgem cinzenta
    namedWindow( "Imagem Cinzenta", cv::WINDOW_AUTOSIZE );
    imshow( "Imagem Cinzenta", gray_img );
    Mat histogramaGray = computeHistogram( gray_img );
    Mat histImageGray = createHistogramImage( histogramaGray );
    namedWindow( "Histograma Cinzenta", cv::WINDOW_AUTOSIZE );
    imshow( "Histograma Cinzenta", histImageGray);
    // Esperar
